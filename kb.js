const questions = [
  ...
  function checkAnswer(answerIndex) {
    ...
    if (answerIndex === correctAnswerIndex) {
      ...
      // Store the total money won in local storage
      totalMoneyWon = prizeList[currentQuestionIndex];
      localStorage.setItem('totalMoneyWon', totalMoneyWon);
  ...
  function resetGame() {
    // Display the result div with username and money won
  ...
  // Wait for 5 seconds before reloading the page
    setTimeout(() => {
      ...
      totalMoneyWon = 0;
      localStorage.removeItem('totalMoneyWon');
  ...
  document.addEventListener('nextQuestion', () => {
    ...
    document.getElementById('fifty-fifty').disabled = false; // Re-enable 50:
});
